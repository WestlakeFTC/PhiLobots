...
void encoderTurn(int degrees){
	...
	int countToTurn = (int)((degrees * robotHalfWidth)/(360*wheelRad))*(cpr)+0.5+ (0.04*cpr); //fancy conversion function we figured out
  allMotorsPower(power);
...
void straightMove(int inches){
	if(inches == 0){return;}
	int countToTurn = (int)((cpr*inches)/(PI*wheelRad*2)+0.5);
	int power = FULL_POWER;
	if(inches < 0){
		power = -power;
	}
