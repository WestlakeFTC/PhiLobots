...
void initializeRobot()
{
	...
	WestCoaster_init(g_wcDrive, FrontL, FrontR, BackL, BackR, FrontL, FrontL);
...
//	WestCoaster_encoderObservedTurn(g_wcDrive,6, 80); //-3 for school
 ...
 goalGrabberUp();
  ...
  writeDebugStreamLine("delta: %d", delta);
	WestCoaster_turnWithMPU(g_wcDrive,abs(delta)+15,90);//10

	WestCoaster_moveStraightWithMPU(g_wcDrive,-25, 90);//-12
	while(!checkLiftDone()){};
  goalGrabberDown();
  WestCoaster_moveStraightWithMPU(g_wcDrive, 15, 80);
