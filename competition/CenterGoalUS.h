 ...
 */
int scanForCenterGoal(WestCoaster& wc, sonar_sensor_t sonarSensor, int search_angle)
{
 ...
 *                The function will try to minimize distance by scanning
 *                for the goal within [-search_angle, search_angle] range
 * @return true if sucess, false if for any reason that this function can not finish.
 */
                ...
                int target_distance, int search_angle)
{
  ...
  static const int MOVE_POWER = 40;
	int distance=scanForCenterGoal(wc, sonarSensor, search_angle);
	...
	while(distance>target_distance+DIST_TOLERANCE){
		...
		distance=scanForCenterGoal(wc, sonarSensor, search_angle);
	}
	return true;
}
