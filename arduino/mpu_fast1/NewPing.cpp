...
NewPing::NewPing(uint8_t trigger_pin, uint8_t echo_pin, unsigned int max_cm_distance) {
	_echoPinInt = echo_pin; // Assign the echo pin/interrupt.
        pinMode(_echoPinInt, INPUT_PULLUP);
...
void NewPing::ping_interrupt(PCIntvoidFuncPtr userFunc) {
	detachPinChangeInterrupt(_echoPinInt);
	intFunc3 = userFunc;
	
	if (!ping_trigger(true)) return; // Trigger a ping, if it returns false, return without starting the echo interrupt.

	attachPinChangeInterrupt(_echoPinInt, userFunc, RISING);
}

void NewPing::ping_interrupt2(PCIntvoidFuncPtr userFunc) {
	_max_time = micros() + _maxEchoTime; // Ping started, set the timeout.
	attachPinChangeInterrupt(_echoPinInt, userFunc, FALLING);
}
